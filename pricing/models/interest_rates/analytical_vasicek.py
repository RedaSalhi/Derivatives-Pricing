...
def vasicek_zero_coupon_price(r_t, t, T, a, lam, sigma, face_value=1.0):  #need extraction from pricing.models.interest_rates.analytical_vasicek import vasicek_zero_coupon_price
    # Add validation
    if T <= t:
        raise ValueError("Maturity T must be greater than current time t")
...
# ------------------------------------------------------------------------------
def generate_yield_curves(r_path, snapshot_times, maturities, a, lam, sigma, dt): #need extraction from pricing.models.interest_rates.analytical_vasicek import generate_yield_curves
    yield_curves = {}
    n = len(r_path)
...
def price_coupon_bond(r0, t, a, lam, sigma, maturity=5, face=1.0, coupon=0.05, dt=0.5): #need extraction from pricing.models.interest_rates.analytical_vasicek import price_coupon_bond
    # Add validation
    if maturity <= t:
        raise ValueError("Maturity must be greater than current time t")
...
def vasicek_bond_option_price(r_t, t, T1, T2, K, a, lam, sigma, face=1.0, option_type='call'):
    # Add validation
    if T1 <= t:
        raise ValueError("Option expiry T1 must be greater than current time t")
    if T2 <= T1:
    ...
    # Calculate B coefficient for the bond price volatility
    B = (1 - np.exp(-a * (T2 - T1))) / a
    
    # Calculate the variance of the bond price at T1
    sigma_P_sq = (sigma**2 / (2 * a**3)) * (1 - np.exp(-a * (T2 - T1)))**2 * (1 - np.exp(-2 * a * (T1 - t)))
    sigma_P = np.sqrt(sigma_P_sq)

    # Calculate d1 and d2 for Black-Scholes type formula
    d1 = (np.log(face * P_t_T2 / (K * P_t_T1)) / sigma_P) + 0.5 * sigma_P
    d2 = d1 - sigma_P
    
    # Calculate option price
    if option_type == 'call':
        option_price = face * P_t_T2 * norm.cdf(d1) - K * P_t_T1 * norm.cdf(d2)
    elif option_type == 'put':
        option_price = K * P_t_T1 * norm.cdf(-d2) - face * P_t_T2 * norm.cdf(-d1)
